...
An alternative is Policy Search (PS) methods \cite{p_search_surv_2011} in which the policy is a parameterised function 
which naturally supports continuous actions. The parameters are updated in order to maximise the 
reward function (gradient descent) without evaluating a value function. Solving continuous POMDPs (both state and action) 
with a large number of policy parameters (up to a point in which we can consider it to be non-parametric) can 
be difficult as the autonomous exploration and gradient 
