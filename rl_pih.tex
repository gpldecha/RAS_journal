...
a Programming by Demonstration (PbD) approach in which we solely provide our FPI algorithm with sample 
episodes demonstrated by human teachers. It has been previously shown \cite{Chambrier2014} 
...
socket and establish a connection. For this task no vision and hearing information is used leaving 
only haptic information for both the teachers and robot apprentice. The haptic information is 
made available via a force-torque sensor mounted on the plug. This choice is motivated by two reasons:
...
simpler to understand when compared with PBVI as there is no need to implement pruning heuristics which are required 
...
In \cite{Boyan95generalizationin} the authored demonstrated that when traditional regression techniques 
...
of the maximisation in VI which we consider infeasible in our case. When controlling a robotic system on-policy 
...
Q-value function is represented by linear weighed sum of radial basis functions and a policy is learned to control 
...
(\textit{Bottom-right}) the KUKA LWR robot is carrying the PiH task that it learned from human demonstrations.
...
the Most Likely State (MLS) ${\hat{x} := \argmax\limits_{x_t} b_t \in \mathbb{R}^3}$ and differential entropy ${U := H(b_t) \in \mathbb{R}}$.
...
one maps a belief feature vector to linear velocity $\pi_{\Param_1} : F \mapsto u$ and the other to maps sensed wrench to angular 
...
The reward of a demonstrated episode $R_e$ is the sum of discounted one-step rewards $r$,
...
the initial demonstrations provided by the teachers. Equation \ref{eq:grad_log_cost} differs with \cite[p.51]{p_search_surv_2011} 
by the fact that we are using the joint instead of the conditional distribution, a result of maximising Equation \ref{eq:expected_reward}.
In most policy search approaches the policy is conditioned on the state space, $\pi_{\Param}(u|F)$. This would lead
...
is the result of \textbf{one iteration} of fitted policy iteration which we call Q-EM. Figure \ref{fig:data-flow} (\textit{Right} Learning)
summaries the learning process for both policies.

This compares the effect of one policy evaluation and improvement sequence have without doing any additional rollouts. This ensures that both policies
...
linear combination of the modes which could result in no displacement as result of cancellation.
...
Figure \ref{fig:policy_vf} illustrates the policy vector field for GMM and Q-EM, both learned from teachers demonstrations. Figure \ref{fig:data-flow} (\textit{Right} Control)
summaries how both linear and angular velocities are generated from $\pi_{\Param_1}$ and $\pi_{\Param_2}$.
