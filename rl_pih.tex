...
Solving continuous action POMDPs with policy search methods, given a large number of policy parameters,
can be difficult as the autonomous exploration, traditionally used in RL, becomes quickly inefficient and 
learning is slow due to large variance in the gradient estimate.
...
Fitted Policy Iteration (FPI) method in which sample episodes (demonstrations)
...
It has been previously shown \cite{Chambrier2014} that humans exhibit both risk-prone and averse behaviour which 
constitutes an ideal training set for RL, removing the need for costly autonomous exploration. 
A belief-space value function is learned from these human demonstrations using a Fitted policy evaluation approach 
and it is used to learn a Gaussian Mixture Model control policy.
...
faster convergence as the critic supplies the actor with low-variance knowledge of the performance. The problem with 
AC methods is that function approximation combined with temporal difference learning can lead to divergence. Fitted
Reinforcement Learning methods are robust against divergence, which we cover next.
...
\subsection{PiH-search experimental setup}
...
\centering
\includegraphics[width=0.9\linewidth]{./Figure/parameters-system-only-two.pdf}
...
The parameters of the linear velocity policy $\pi_{\Param_1}$ are learned in a Fitted Policy Iteration framework
...
Policy Iteration is an on-policy reinforcement learning method which through successive iterations of policy \textit{evaluation} and \textit{improvement} 
both value function and policy converge to their optimal parameters \cite[Chap. 4.6]{sutton1998reinforcement}.
...
A reward of ${r=0}$ is assigned at each time step until the goal (plug-socket connection) is achieved, where a reward of $r_{T}=100$ is obtained. 
Given the continuous nature of the belief feature vector Locally Weighted Regression (LWR) \cite{Atkeson97locallyweighted}  is used as a function 
approximator of the value function.

To learn the value function Fitted/Batch RL approach is taken\cite{EGW05}, an offline method which applies multiple sweeps of the Bellman backup operator 
to a dataset of tuples $\{(F_t,r^{[i]}_{t},F_{t+1})\}_{i=1,\cdots,M}$ until the Bellman residual, ${||\hat{V}^{\pi}_{k+1}(F) - \hat{V}^{\pi}_{k}(F)||}$, converges, 
see Algorithm \ref{alg:fpe}.
...
\Output{$\hat{V}^{\pi}_{k}(F_t)$}
\BlankLine
\While{$||\hat{V}^{\pi}_{k+1}(\B) - \hat{V}^{\pi}_{k}(\B)|| > \epsilon$}{
  $\hat{V}^{\pi}_{k+1}(F_t)$ = Regress($F$, $r_t + \gamma \hat{V}^{\pi}_k(F_{t+1}))$
}
...
of our problem we assume this to be unrealistic and we opt for an on-policy approach mentioned above.
