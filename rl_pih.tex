...
in an Actor-Critic Fitted Policy Iteration (FPI) framework. 
A belief-space critic is learned offline in a Fitted RL framework from trajectory data demonstrated by a group of 
...
a Point Mass Filter (PMF). The critic is then used to train the actor policy represented as Gaussian Mixture Model (GMM).
Evaluations performed both in simulation and on the KUKA LWR robot showed that the proposed algorithm outperforms 
...
% ---------- What is the problem ? ----------
% Cannot solve original POMDP problem!
%
%	o approximate techiques require a discretisation of the belief space.
%	-> this is not good for tasks which are naturally continuous
%
%	o gradient methods are suitable when the number of parameters of the policy are small with 
%	  respect to the size of th environment. 
%	-> structure of the policy would require a precisable amout of domain knowledge.
%
%	o Actor-Critic allows the formulation both a continuous action and space representation.
%	 -> But to guarantee convergence during online learning both action an policy parameterisation is restrictive
%
% Our solution:
%
%	o Offline batch reinforcement learning.
%
...
%in which the underlying state space is not directly observable. A POMDP is defined by a tuple $\{S, A, O, T, Z, R,\gamma\}$, 
%where $S$ is  the state space, $A$ is the action space, $O$ is the observation space, $T = p(s'|s,a)$ is the state space transition model,
%$Z = p(o|s,a)$ is the observation model,  $R(s,a)$ is the reward model and $\gamma \in (0,1]$ the discount factor used to bound the reward function 
%for infinite-horizon decision problems. As the state space is not observable it is common to cast the POMDP into a belief or information state MDP [cite]
%which is defined by the tuple ${B,A,\tau,R,\gamma}$ where $B$ is the space of probability distributions over the state space and $\tau(b_t,y_t,a_t)$ is 
%the belief state transition function.

% To resolution of a POMDP consists of finding a policy which maximised the expected reward $\mathbb{E}[]$
%(Astrom, 1965)
% Because the state space is partially observable the expected reward has to be computed 
% for each possible history of states, actions and observations. All approaches in the literature instead encapsulate all these possible histories
...
\begin{equation} \label{eq:optimal_value_f}
  V^{\pi_{\Param}}(b) = R_B(b) + \sum\limits_{u}\, \pi_{\Param}(b,u)\, \gamma \, \sum\limits_{y} p(y|b,a)\, V^{\pi_{\Param}}(\tau(b,a,y))
\end{equation}

% Need to throw in the fact that POMDP is piece wise linear and convex.
Solving discrete state space POMDP problems is a difficult as the number of parameters of the value function grows 
at a rate exponential with respect to the decision horizon \cite[Chap. 15]{Thrun_Burgard_Fox_2005}\cite{Sondik_1973} doe 
to the efforts to preserve the Piece Wise Linear and Convex PWLC of the value function. Recent research has focused 
on pruning, belief selection and exploration strategies \cite{SARSOP}, \cite{POMDP_approach_2010}, methods known as 
Point-based Value Iteration (PBVI). However these Value Iteration (VI) approaches are not easily transferable to a 
continuous domains [cite] such as 7 degree of freedom robot. 

When learning a POMDP policy for a robotic system it is typical to use Policy Search (PS) methods [cite]
were the parameters of the policy are directly optimised to maximise the reward function without 
estimating a value function. Such approach yield a smooth policy, however the POMDP solution is only 
guarantee to be local. Policy search methods work well when the number of parameters are relatively low but 
as the number of parameters increase the learning becomes slow. In our task we do not want to make any assumptions
regarding the structure of the policy and as a result should be non-parametric. 

Actor-critic (AC) methods \cite[Chap. 6.6]{sutton1998reinforcement} have two separate parameterisations, one for 
the policy (actor) and one for the value function (critic). The critic is used to update both the policy and 
value function parameters such to maximise the expected reward. Having a separate policy and value function
guarantees a smooth control output, ideal for a robotic controller, whist faster convergence when compared 
to PS methods  \cite{rl_ac_surv_2012}.


%ACs have been reported and proven \cite{rl_ac_surv_2012} to 
%be faster than policy search methods, as the variance in the gradient estimate is smaller. Their flexibility allows to 
%take both advantage 

%An Actor-Critic (AC) framework allows to have a smooth continuous policy whilst the critic (value function) allows 
%to retain witnessed.

% Learning value function and making the policy its derivative: because very discontinuous movements not good [cite],
% this renders most of the above approaches ill-suited. 

% Policy search methods in which the parameters of a policy are directly optimised to maximise the reward function without 
% evaluating the value function tipically require the number of parameters to be orders of magnitude smaller than the state space [cite].
%
% In an Actor-Critic (AC) formulation, Learning a value function and using it to improve the paramters of a policy cab reduced the variance of the gradient 
% estimate and speed up the learning. 
...
fixes the state space parameters $\MuK_{\B}$, $\SigK_{\B\B}$ (and partially $\SigK_{\U\B}$) and thus greatly constrains the solution.
Instead Equation \ref{eq:grad_log_cost} is optimised with respect to the joint distribution $\pi_{\Param}(\U_t,\B_t)$ and not 
the conditional $\pi_{\Param}(\U_t|\B_t)$. This has two benefits. Firstly, the input dimensions (the state space) are no 
...
the responsibility factor, denoting the probability that data point $\mathbf{x}^{[j]} = [\dot{x}^{[j]},b^{[j]}]^{\mathrm{T}}$ belongs to  Gaussian function $k$.}
\end{figure}
