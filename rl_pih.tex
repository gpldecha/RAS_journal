...
If not considered appropriately by a policy or planner can lead to either sub-optimal task execution or failure. 
We consider a Peg-in-Hole (PiH) task in which both a human and robot apprentice must be able to find a socket 
and connect a power plug to it without using any vision, making the state space partially observable, whilst solely relaying on haptic information.
A search policy can be obtained by encoding the search tasks as a Partially Observable Markov Decision Process (POMDP) 
which is subsequently solved by dynamic programming. This quickly becomes infeasible for continuous state and action spaces do 
the curse of history and dimensionality which autonomous exploration-exploitation strategies are sensitive to.  
We address this problem by demonstrating how human intuition can be leveraged in an Actor-Critic Fitted Policy Improvement (FPI) framework. 
A group of human teachers demonstrate the PiH-search task whilst blindfolded. The position 
uncertainty, represented by a Point Mass Filter (PMF), is recorded and compressed to the most likely state and entropy. 
A belief space value function is learned in a Fitted RL framework is learned offline and used to update the parameters 
of a Gaussian Mixture Model (GMM) policy. The GMM Actor-Critic policy, called Q-EM, is compared with both a Greedy and non-optimised GMM policy for the distance taken 
